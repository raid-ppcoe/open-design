...
export function registerPluginRoutes(app: Express, deps: RegisterPluginRoutesDeps): void {
  ...
  app.get('/api/applied-plugins', readRateLimit(), (_req, res) => { try { const rows = db.prepare(`SELECT id FROM applied_plugin_snapshots ORDER BY applied_at DESC LIMIT 500`).all() as SqliteRowId[]; res.json({ snapshots: rows.map((r) => plugins.getSnapshot(db, r.id)).filter((x): x is AppliedPluginSnapshotLike => x !== null) }); } catch (err) { res.status(500).json({ error: String(err) }); } });
  app.get('/api/projects/:projectId/applied-plugins', readRateLimit(), (req, res) => { try { const rows = db.prepare(`SELECT id FROM applied_plugin_snapshots WHERE project_id = ? ORDER BY applied_at DESC`).all(req.params.projectId) as SqliteRowId[]; res.json({ snapshots: rows.map((r) => plugins.getSnapshot(db, r.id)).filter((x): x is AppliedPluginSnapshotLike => x !== null) }); } catch (err) { res.status(500).json({ error: String(err) }); } });
...
export function registerProjectPluginRoutes(app: Express, deps: RegisterPluginRoutesDeps): void {
  ...
  app.post('/api/projects/:id/plugin-candidates/:candidateId/dismiss', writeRateLimit(), (req, res) => { if (!helpers.isLocalSameOrigin(req, helpers.resolvedPortRef.current)) return res.status(403).json({ error: 'cross-origin request rejected' }); const candidate = plugins.dismissSkillPluginCandidate(db, String(req.params.id), String(req.params.candidateId)); if (!candidate) return helpers.sendApiError(res, 404, 'NOT_FOUND', 'plugin candidate not found'); if (candidate.assistantMessageId) db.prepare(`DELETE FROM messages WHERE id = ?`).run(candidate.assistantMessageId); res.json({ ok: true, candidate }); });
  ...
  app.post('/api/plugins/share-tasks/:id/wait', writeRateLimit(), (req, res) => {
    ...
    const task = helpers.pluginShareTaskStore.get(String(req.params.id));
